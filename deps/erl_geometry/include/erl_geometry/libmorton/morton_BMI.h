...
namespace libmorton {
    ...
    namespace bmi2_detail {
        inline uint32_t
        pdep(uint32_t source, uint32_t mask) noexcept {
            return _pdep_u32(source, mask);
        }

        inline uint64_t
        pdep(uint64_t source, uint64_t mask) noexcept {
            return _pdep_u64(source, mask);
        }

        inline uint32_t
        pext(uint32_t source, uint32_t mask) noexcept {
            return _pext_u32(source, mask);
        }

        inline uint64_t
        pext(uint64_t source, uint64_t mask) noexcept {
    ...
    inline morton
    m2D_e_BMI(const coord x, const coord y) {
        morton m = 0;
    ...
    template<typename morton, typename coord>
    inline void
    m2D_d_BMI(const morton m, coord &x, coord &y) {
    ...
    template<typename morton, typename coord>
    inline morton
    m3D_e_BMI(const coord x, const coord y, const coord z) {
        morton m = 0;
    ...
    template<typename morton, typename coord>
    inline void
    m3D_d_BMI(const morton m, coord &x, coord &y, coord &z) {
