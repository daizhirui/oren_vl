...
namespace libmorton {
    ...
    inline morton
    m2D_e_BITALG(const coord x, const coord y) {
        return bitalg_detail::bitzip2D(static_cast<coord>(x), static_cast<coord>(y));
    }

    template<typename morton, typename coord>
    inline void
    m2D_d_BITALG(const morton m, coord &x, coord &y) {
        bitalg_detail::bitunzip2D(m, x, y);
    }

    template<typename morton, typename coord>
    inline morton
    m3D_e_BITALG(const coord x, const coord y, const coord z) {
    ...
    template<typename morton, typename coord>
    inline void
    m3D_d_BITALG(const morton m, coord &x, coord &y, coord &z) {
