...
namespace libmorton {
    ...
    inline bool
    findFirstSetBitZeroIdx(const morton x, unsigned long *firstbit_location) {
        ...
        if (x == 0) {
            ...
            *firstbit_location = static_cast<unsigned long>(
                (sizeof(unsigned long long) * 8) - __builtin_clzll(x) - 1);
            return true;
        }
#endif
    }
    ...
    template<typename morton>
    inline bool
    findFirstSetBit(const morton x, unsigned long *firstbit_location) {
