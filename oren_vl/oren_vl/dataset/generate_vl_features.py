...
class VLFeatureConfig(ConfigABC):
    ...
    extracter_kwargs: Optional[dict] = None  # genuinely optional: None == {}
    input_dir: str = None
...
def _load_traj(path: str, expected_n: int) -> np.ndarray:
    ...
    assert arr.shape[1] == 16, (
        f"Expected 16 numbers per row in {path}, got {arr.shape[1]}"
    )
    assert arr.shape[0] == expected_n, (
        f"{path} has {arr.shape[0]} poses but found {expected_n} RGB frames"
    )
...
def generate_vl_features(cfg: VLFeatureConfig):
    ...
    extracter_module = importlib.import_module(f"oren_vl.vl_features.{cfg.extracter_name}")
    ...
    # Probe feature shape with a single forward pass.
    ...
    assert depth0.shape[-2:] == (h_feat, w_feat), (
        f"depth_transform produced {tuple(depth0.shape[-2:])}, " f"but feature map is {(h_feat, w_feat)}"
    )
    ...
    feat_mm = np.memmap(
        ...
        dtype=np.float16,
        mode="w+",
        shape=(n_frames, channels, h_feat, w_feat),
    )
    depth_mm = np.memmap(
    ...
    with torch.no_grad():
        for idxs, rgbs, depths in tqdm(loader, desc="Extracting VL features", ncols=80):
            ...
            feats_np = feats.detach().half().cpu().numpy()
